...
login = function(username, password=NA, domain) {
	...
	loginRequestSucceeded = res$status_code == 200
	currentUserSucceeded = currentUser()  # User greeting

	# On older server versions in combination with some older HTTR versions login request can return 404 even though request was successful
	# In those cases successful currentUser request serves as authentication verification
	# Starting from 1.8 it shouldn't be necessary
	loggedIn = loginRequestSucceeded || currentUserSucceeded
